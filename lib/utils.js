...
// Subscribe to new messages for a session. The callback receives payload.new.
...
// Subscribe to session status updates (e.g., ended). The callback receives payload.new.
...
const RATE_LIMITS = {
  FIND_CHAT: 10000, // 10 seconds between find chat attempts
  LEAVE_CHAT: 30000, // 30 seconds between leave chat attempts
};

/**
 * Check if an action is rate limited
 * @param {string} action - The action name (e.g., 'FIND_CHAT', 'LEAVE_CHAT')
 * @returns {{ allowed: boolean, remainingMs: number }} - Whether action is allowed and remaining cooldown in ms
 */
export function checkRateLimit(action) {
  const cooldownMs = RATE_LIMITS[action];
  if (!cooldownMs) {
  ...
  const elapsed = now - parseInt(lastActionTime, 10);
...
/**
 * Format remaining milliseconds into a user-friendly string
 * @param {number} ms - Milliseconds
 * @returns {string} - Formatted string like "2.5s" or "1.2s"
 */
